...
class RespirationRepository:
    def __init__(self, pool: asyncpg.Pool):
        self.pool = pool
        self.table_name = "t_respiration"
        self.measure_table_name = "t_measure_respiration"

    async def get_by_id(self, idt_respiration: int) -> Optional[Respiration]:
        async with self.pool.acquire() as conn:
            query = f"SELECT * FROM {self.table_name} WHERE idt_respiration = $1"
            result = await conn.fetchrow(query, idt_respiration)
            if result:
                return Respiration(*result)
            return None

    async def get_by_pet_id(self, idt_pet: int) -> List[dict]:
        async with self.pool.acquire() as conn:
            query = f"""
                SELECT idt_respiration as id, respiration as rate, respiration_dt as measured_at, 'manual' as type
                FROM {self.table_name}
                WHERE idt_pet = $1
                UNION ALL
                SELECT idt_measure_respiration as id, phase as rate, create_dt as measured_at, 'device' as type
    ...
    async def get_by_pet_id_and_date(self, idt_pet: int, start_date: datetime) -> List[dict]:
        async with self.pool.acquire() as conn:
            query = f"""
                SELECT idt_respiration as id, respiration as rate, respiration_dt as measured_at, 'manual' as type
                FROM {self.table_name}
                WHERE idt_pet = $1 AND respiration_dt >= $2
                UNION ALL
                SELECT idt_measure_respiration as id, phase as rate, create_dt as measured_at, 'device' as type
                FROM {self.measure_table_name}
